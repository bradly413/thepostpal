...
function ImagePlaceholder({ format }: { format: PostFormat }) {
  ...
  return (
    <div
      className="w-full relative rounded-xl bg-gradient-to-br from-white/[0.06] to-white/[0.03] flex items-center justify-center"
...
function FloatingImage({ imageUrl, format, glow }: { imageUrl?: string; format: PostFormat; glow: string }) {
  if (!imageUrl) return <ImagePlaceholder format={format} />;
  return (
    <div className="relative">
      {/* Glow behind */}
      <div
        className="absolute -inset-3 rounded-2xl blur-2xl opacity-40"
        style={{ background: glow }}
      />
      <img
        src={imageUrl}
        alt=""
        className="relative w-full rounded-xl shadow-2xl shadow-black/50"
        style={{ aspectRatio: `${FORMAT_RATIOS[format].w}/${FORMAT_RATIOS[format].h}`, objectFit: "cover" }}
      />
    </div>
  );
}
...
function InstagramMockup({ format = "square", imageUrl, caption, username, avatarUrl, hashtags, likes, comments, timestamp }: Omit<SocialMockupProps, "platform">) {
  ...
  if (format === "story") {
    return (
      <div className="relative rounded-2xl overflow-hidden shadow-2xl shadow-black/50" style={{ aspectRatio: "9/16", maxHeight: 480 }}>
        {imageUrl ? (
    ...
    <div>
      ...
      <div className="flex items-center justify-between px-1 mb-3">
        <div className="flex items-center gap-2.5">
          <div className="w-9 h-9 rounded-full overflow-hidden ring-2 ring-[#E1306C]/30">
            {avatarUrl ? <img src={avatarUrl} alt="" className="w-full h-full object-cover" /> : <DefaultAvatar color="#E1306C" />}
          </div>
          <span className="text-white/90 text-[13px] font-semibold">{displayName}</span>
        </div>
      ...
      {/* Floating image */}
      <FloatingImage imageUrl={imageUrl} format={format} glow="linear-gradient(135deg, #833AB4, #E1306C, #F77737)" />

      {/* Actions */}
      <div className="mt-3 px-1">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-4">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth={1.5} className="opacity-60 hover:opacity-100 transition-opacity cursor-pointer">
              <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
            </svg>
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth={1.5} className="opacity-60">
        ...
        <p className="text-white/80 text-[13px] font-semibold mb-1">{displayLikes} likes</p>
        <p className="text-white/60 text-[13px] leading-relaxed">
          <span className="font-semibold text-white/80">{displayName}</span>{" "}
          {displayCaption}
        </p>
        {hashtags && <p className="text-[#1DA1F2]/60 text-xs mt-1">{hashtags}</p>}
        <p className="text-white/20 text-[10px] mt-1.5">View all {displayComments} comments</p>
        <p className="text-white/15 text-[10px] mt-1 uppercase tracking-wide">{displayTime}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-3 px-1">
        <div className="w-10 h-10 rounded-full overflow-hidden ring-2 ring-[#1877F2]/30">
          {avatarUrl ? <img src={avatarUrl} alt="" className="w-full h-full object-cover" /> : <DefaultAvatar color="#1877F2" />}
        </div>
      ...
      {/* Caption */}
      {displayCaption && <p className="text-white/70 text-[13px] leading-relaxed mb-3 px-1">{displayCaption}</p>}

      {/* Floating image */}
      <FloatingImage imageUrl={imageUrl} format={format} glow="linear-gradient(135deg, #1877F2, #0C63D4)" />
      ...
      <div className="mt-3 px-1">
        <div className="flex items-center justify-between pb-2.5 border-b border-white/[0.06]">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-white/35 text-[11px]">{displayLikes}</span>
          </div>
          <div className="flex gap-3 text-white/25 text-[11px]">
        ...
        <div className="flex items-center justify-around py-2">
          {[
            ...
            <button key={a.label} className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-white/35 hover:bg-white/[0.04] transition-colors">
...
function LinkedInMockup({ format = "square", imageUrl, caption, username, avatarUrl, likes, comments, timestamp }: Omit<SocialMockupProps, "platform">) {
  ...
  return (
    <div>
      {/* Header */}
      <div className="flex items-center gap-3 mb-3 px-1">
        <div className="w-10 h-10 rounded-full overflow-hidden ring-2 ring-[#0A66C2]/30">
          {avatarUrl ? <img src={avatarUrl} alt="" className="w-full h-full object-cover" /> : <DefaultAvatar color="#0A66C2" />}
        </div>
        <div className="flex-1">
          <span className="text-white/90 text-[13px] font-semibold block">{displayName}</span>
          <span className="text-white/25 text-[10px] block">Real Estate Professional</span>
      ...
      {/* Caption */}
      <p className="text-white/60 text-[13px] leading-relaxed mb-3 px-1">{displayCaption}</p>

      {/* Floating image */}
      <FloatingImage imageUrl={imageUrl} format={format} glow="linear-gradient(135deg, #0A66C2, #004182)" />
      ...
      <div className="mt-3 px-1">
        <div className="flex items-center justify-between pb-2 border-b border-white/[0.06]">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-white/25 text-[10px]">{displayLikes}</span>
          </div>
          <span className="text-white/20 text-[10px]">{displayComments} comments</span>
        </div>

        <div className="flex items-center justify-around py-1.5">
          {["Like", "Comment", "Repost", "Send"].map((label) => (
            <button key={label} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-white/25 text-[11px] font-medium hover:bg-white/[0.04] transition-colors">
              {label}
            </button>
          ))}
        </div>
  ...
  return (
    <div>
      <div className="flex gap-2.5 px-1">
        <div className="w-9 h-9 rounded-full overflow-hidden shrink-0 ring-2 ring-[#1DA1F2]/30">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-white/65 text-[13px] leading-relaxed mb-3">{displayCaption}</p>

          {/* Floating image */}
          <FloatingImage imageUrl={imageUrl} format={format} glow="linear-gradient(135deg, #1DA1F2, #0C85D0)" />

          {/* Actions */}
          <div className="flex items-center justify-between pt-3 pb-1 text-white/25">
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4 px-1">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[12px] font-semibold text-white/50 tracking-wide">{config.name} Preview</span>
        </div>
          ...
          {config.formats.map((f) => (
            <button
              ...
              className={`px-2.5 py-1 rounded-md text-[10px] font-medium transition-all ${
                activeFormat === f
                  ? "bg-white/[0.10] text-white/70 shadow-sm"
